...
test("the API hard stop precedes every post-preview ceremony and mutation boundary", () => {
  ...
  assert.doesNotMatch(source, /runner-evidence|createRunnerEvidenceClient|runner-job|createRunnerJobService|acquireInitial|reacquire|runnerEvidenceConfig|runnerCapabilityProviderAvailable/);
  const capability = readFileSync(new URL("../lib/runner-capability.ts", import.meta.url), "utf8");
  assert.doesNotMatch(capability, /process\.env|runner-evidence|createRunnerEvidenceClient|runner-job|createRunnerJobService/);
