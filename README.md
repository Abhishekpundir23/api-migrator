...
| `npm run test:ops` | Checks the shell, gateway, host-deployment, hosted-smoke data/workflow, and runner-image contracts without claiming an authoritative Linux security drill |
...
GitHub Actions runs `npm run ci` on pushes and pull requests. The separate Linux L7 workflow also runs the 15-scenario GitHub-hosted smoke when publication-runner paths change. CI uses only its read-only checkout token, does not persist that credential, does not configure application GitHub credentials, push branches, open real PRs, or prove customer-repository compatibility. Hosted-smoke output remains co-resident, self-attested regression evidence; a real pilot still requires operator review and externally observed evidence from approved repositories.
